      ...
      <div class="card-body">
        <form action="{{ route('slider.update', ['slider' => $slider->id]) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <input type="text" value="{{ old('title', $slider->title) }}" class="form-control" id="title" name="title" required>
            @error('title'){{$message}}@enderror
          </div>

          <div class="mb-3">
            <label for="link_title" class="form-label text-white">عنوان لینک</label>
            <input type="text" value="{{ old('link_title', $slider->link_title) }}" class="form-control" id="link_title" name="link_title" required>
            @error('link_title'){{$message}}@enderror
          </div>

          <div class="mb-3">
            <label for="link_adress" class="form-label text-white">آدرس لینک</label>
            <input type="url" value="{{ old('link_adress', $slider->link_adress) }}" class="form-control" id="link_adress" name="link_adress" required>
            @error('link_adress'){{$message}}@enderror
          </div>

          <div class="mb-3">
            <label for="body" class="form-label text-white">متن اسلایدر</label>
            <textarea class="form-control" id="body" name="body" rows="4" required>{{ old('body', $slider->body) }}</textarea>
