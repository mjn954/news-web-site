      ...
      <div class="card-body">
        <form action="{{ route('Culturalnews.update', ['Culturalnews' => $Culturalnews->id]) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <input type="text" value="{{ old('title', $Culturalnews->title) }}" class="form-control" id="title" name="title" required>
            @error('title')<div class="text-danger">{{ $message }}</div>@enderror
          </div>

          <div class="mb-3">
            <label for="body" class="form-label text-white">متن خبر</label>
            <textarea class="form-control" id="body" name="body" rows="4" required>{{ old('body', $Culturalnews->body) }}</textarea>
          ...
          <div class="mb-3">
            ...
            @if($Culturalnews->image)
              <img id="imagePreview" src="{{ asset('storage/' . $Culturalnews->image) }}" alt="تصویر انتخاب شده" class="img-fluid mt-2">
            @endif
          </div>
