...
<body>
  <div class="container mt-5">
    <div class="card">
      ...
      <div class="card-body">
        <form action="{{ route('Sportsslider.update', ['Sport' => $Sport->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PUT")
          ...
          <div class="mb-3">
            ...
            <input type="text" value="{{ old('title', $Sport->title) }}" class="form-control" id="title" name="title" required>
            @error('title')<div class="text-danger">{{$message}}</div>@enderror
          </div>

          <div class="mb-3">
            <label for="link_title" class="form-label text-white">عنوان لینک</label>
            <input type="text" value="{{ old('link_title', $Sport->link_title) }}" class="form-control" id="link_title" name="link_title" required>
          ...
          <div class="mb-3">
            ...
            <input type="url" value="{{ old('link_adress', $Sport->link_adress) }}" class="form-control" id="link_adress" name="link_adress" required>
            @error('link_adress')<div class="text-danger">{{$message}}</div>@enderror
          </div>

          <div class="mb-3">
            <label for="body" class="form-label text-white">متن اسلایدر</label>
            <textarea class="form-control" id="body" name="body" rows="4" required>{{ old('body', $Sport->body) }}</textarea>
