...
<body>
    <div class="container mt-5">
        <div class="header">
            <h1>{{ $sportnews->title }}</h1>

            <!-- بررسی اینکه تاریخ انتشار وجود دارد یا نه -->
            <p>تاریخ انتشار:
                @if($sportnews->created_at)
                    {{ $sportnews->created_at->format('d F Y') }}
                @else
                    تاریخ انتشار نامشخص
                @endif
            </p>
        ...
        <div class="news-container mt-4">
            ...
            <img src="{{ $sportnews->image ? asset('storage/' . $sportnews->image) : 'https://via.placeholder.com/600x400.png?text=No+Image' }}" alt="News Image" class="img-fluid"/>
